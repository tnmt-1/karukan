...
/// Convert hiragana text to half-width katakana (F8 conversion).
///
/// Fork-ported: upstream removed this convenience wrapper in its rewriter
/// consolidation (#58); the F6-F10 function-key feature depends on it.
...
/// Convert kana (hiragana/katakana) text to Hepburn romaji (F9/F10 conversion).
///
/// Fork-ported: standard IME F9/F10 convert the composing text to
/// romanized alphanumerics, and upstream does not provide a kana→romaji
/// utility. Sokuon (っ) doubles the following consonant, ん is `n` (with
/// apostrophe before a vowel/y), and small-kana digraphs follow Hepburn
/// (しゃ→sha). Characters outside the tables pass through unchanged.
