...
| キー | 動作 |
|------|------|
| 文字キー | ローマ字入力 → ひらがな変換（単語末の `n` も確定時に `ん` になります） |
| Space / Tab / ↓ | かな漢字変換を開始 |
| 変換キー (Henkan) | かな漢字変換を開始（JISキーボード） |
| Enter | ひらがなのまま確定 |
| Escape | 候補ウィンドウが開いていれば閉じる → もう一度で入力をキャンセル（2段階） |
| Backspace | 1文字削除 |
| Delete | カーソル位置の文字を削除 |
| ← → | カーソル移動 |
| Home / End | カーソルを先頭 / 末尾に移動 |
| Ctrl+K | カタカナモードに切り替え（空状態でも有効） |
| Ctrl+J | ひらがなに変換（F6相当） |
| Ctrl+L | 全角英数に変換（F9相当） |
| Ctrl+; | 半角英数に変換（F10相当） |
| Ctrl+Space | 全角スペースを入力（コンポーズ中のみ。空状態ではOSへ素通り） |
| かな/カナキー (Hiragana_Katakana) | ひらがな ⇔ カタカナモード切替（JISキーボード） |
| テンキー | 直接入力（数字は候補選択に使われず、`/` や `.` もローマ字変換されない） |
...
かなモードは全角のコンテキストなので、ASCII記号はプリエディットで全角になります。Mozcの既定（`preedit_character_form = FULL_WIDTH`）に合わせた挙動です。半角がほしいときは、そのまま変換すると候補一覧に半角形が出ます。

- 日本語固有の記号に置き換わるキー
  - `,` → `、`、`.` → `。`、`-` → `ー`、`~` → `〜`
  - `[` → `「`、`]` → `」`、`/` → `・`
  - `z` 始まりの特殊記号（`z.` → `…`、`z[` → `『`、`zh` → `←` など）
- 全角形になるキー
  ...
  素直な U+FFxx にならないものが3つあります。`"` → `”`、`'` → `’`、`\` → `￥`
- 例外
  - `:` は空状態では絵文字ショートコードの起動キーなので、`：` にはなりません。入力途中の `:` は `：` になります
  - 数字と英字は半角のままです。全角にしたいときは F9（`Ctrl+L`）を使ってください
  - テンキーは直接入力なので、`/` や `.` は変換されません
